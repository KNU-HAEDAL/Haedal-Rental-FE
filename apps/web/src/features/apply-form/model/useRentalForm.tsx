...
export const useRentalForm = () => {
  ...
  const [itemName, setItemName] = useState(() => getStoredValue('itemName', ''));
  const [selectedCategory, setSelectedCategory] = useState(() =>
    getStoredValue('selectedCategory', '')
  );
  const [rentalPeriod, setRentalPeriod] = useState(() => getStoredValue('rentalPeriod', ''));
  const [date, setDate] = useState<Date | undefined>(() => {
    const storedDate = getStoredValue('date', '');
    return storedDate ? new Date(storedDate) : undefined;
  });
  const [image, setImage] = useState<string | null>(() => getStoredValue('image', null));
